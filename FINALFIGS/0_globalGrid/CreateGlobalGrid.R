...
pdf("FINALFIGS/0_globalGrid/CreateGlobalGrid.pdf")
...
df.globe <- df.globe[,-3]
save(df.globe,file="FINALFIGS/0_globalGrid/df.globe.Rda") 

##########################################################
### 2) create Geographic sources in the native range
##########################################################
rm(list=ls())
...
pdf("FINALFIGS/0_globalGrid/CreateGlobalGrid_sources.pdf")
...
write.csv(out,"FINALFIGS/0_globalGrid/df.globe_source.csv",quote=F,row.names = F)
