...
nat2 = nat[!nat.reg=="Kagoshima",]
nat.reg2 = as.factor(nat.reg[!nat.reg=="Kagoshima"])

m = dapc(nat2,n.pca=10,grp = nat.reg2)
m_predict = predict(m,non)
table(m_predict$assign,non.pop)
table(m_predict$assign,non.reg)

pdf("FINALFIGS/3_PCA/DAPC.pdf"); scatter(m); dev.off()
