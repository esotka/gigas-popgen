...
meta <- as.data.frame(read_xlsx("Miura2006haplotypes_meta.xlsx",sheet = 1))
...
write.csv(meta,"Miura2006haplotypes_meta_wSourceIDs.csv",quote=F,row.names = F)
...
dat <- as.data.frame(read_xlsx('Miura2006haplotypes_haps.xlsx',sheet=1))
...
save(native,file="battrForRangerCrossValidation.rda")

### ran on "XXXXCrossvalidation_forServer.R" separately 
load(file="battrRanger_out.rda")
...
mat <- as.matrix(tbl)
mat <- mat+0.01
cols.to.use <- c(blue2red(5)[-1],"black","lightgrey","darkgrey")
...
write.csv(n,"battr_sampleSize.csv",quote=F)
