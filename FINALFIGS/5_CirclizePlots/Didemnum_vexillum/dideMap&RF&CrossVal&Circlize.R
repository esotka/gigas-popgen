...
meta <- as.data.frame(read_xlsx("Stefaniak_AppTable1_summary.xlsx",sheet = 1))
...
#dat <- as.data.frame(read_xlsx('Stefaniak_AppTable1_summary.xlsx',sheet=1))
...
save(native,file="dideForRangerCrossValidation.rda")

### ran on "XXXXCrossvalidation_forServer.R" separately 
load(file="dide_Ranger_out.rda")
...
#mat <- as.matrix(tbl)
mat = as.matrix(data.frame(Europe=rowSums(tbl[,1:2]),nAM_north=tbl[,3],nAM_south=tbl[,4]))
mat <- mat+0.01
cols.to.use <- c(blue2red(5)[-(4:5)],"black","lightgrey","darkgrey")
...
write.csv(n,"dide_sampleSize.csv",quote=F)
