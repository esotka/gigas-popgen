...
meta <- read.csv("gverm_meta.csv")
meta = meta[!meta$long<(-130),] # remove "akg","acr", "asb"
...
dat <- read.csv("gvermMTDNA.606ind.csv")
dat = dat[!dat$popID%in%c("akg","acr", "asb"),] # removed 20 samples
...
save(native,file="gvermMTDNAForRangerCrossValidation.rda")

### ran on "XXXXCrossvalidation_forServer.R" separately 
load(file="gvermMTDNA_Ranger_out.rda")
...
mat <- as.matrix(tbl)
mat <- mat+0.01
cols.to.use <- c(blue2red(5),"black","darkgrey","black","lightgrey")
#cols.to.use <- c(blue2red(5),"black",rep("grey",ncol(mat))) # "black",
...
write.csv(n,"GvermMTDNA_sampleSize.csv",quote=F)
