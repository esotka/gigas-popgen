...
meta <- read_xlsx("Table1_edited.xlsx")
...
dat <- read_xlsx("Table1_edited.xlsx")[,c("Population","A","B","C","D","E","F","G","H")]
dat[is.na(dat)] = 0
pop <- dat$Population
...
save(native,file="hamiforRangerCrossValidation.rda")

### ran on "XXXXCrossvalidation_forServer.R" separately 
load(file="hami_Ranger_out.rda")
...
mat <- as.matrix(tbl)
PNW = rowSums(mat[,c(2,6)])
soEur = rowSums(mat[,c(3,5)])
mat = cbind(mat[,1],PNW,soEur)
colnames(mat) = c("Cali","PNW","soEur")
mat <- mat+0.01
cols.to.use <- c(blue2red(5),"black","darkgrey","lightgrey","black")
...
dev.off()
write.csv(as.matrix(tbl),"hamiByReg.csv")
...
write.csv(n,"hami_sampleSize.csv",quote=F)
