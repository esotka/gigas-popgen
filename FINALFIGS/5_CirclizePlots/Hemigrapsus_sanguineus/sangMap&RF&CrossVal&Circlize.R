...
meta <- as.data.frame(read_xlsx("Blakeslee_Table1.xlsx",sheet = 1))
...
dat <- as.data.frame(read_xlsx('Blakeslee_TableS2+Germany.xlsx',sheet=1))
md <- melt(dat,id="Hap")
...
save(native,file="sangForRangerCrossValidation.rda")

### ran on "XXXXCrossvalidation_forServer.R" separately 
load(file="sang_Ranger_out.rda")
...
write.csv(n,"sang_sampleSize.csv",quote=F)
