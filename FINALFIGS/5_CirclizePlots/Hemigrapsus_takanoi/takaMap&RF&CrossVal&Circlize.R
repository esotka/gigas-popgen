...
meta <- as.data.frame(read_xlsx("Makino 2018 Table S1.xlsx",sheet = 1))[,1:6]
...
dat <- read.table('Makino 2018 hap freq.txt',sep="\t",header=T)
md <- dat[!dat$pop=="pop1",]
#md <- melt(dat,id="Hap")
...
save(native,file="takaForRangerCrossValidation.rda")

### ran on "XXXXCrossvalidation_forServer.R" separately 
load(file="taka_Ranger_out.rda")
...
#mat2 <- as.matrix(tbl)
mat = as.matrix(rowSums(tbl)); colnames(mat) = "Europe"
mat <- mat+0.01
cols.to.use <- c(blue2red(5),"black","black")
...
write.csv(n,"taka_sampleSize.csv",quote=F)
