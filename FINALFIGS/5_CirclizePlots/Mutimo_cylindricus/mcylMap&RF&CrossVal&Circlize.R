...
meta <- read.delim("Hanyuda et al 2018 Supp TableS9_Mutimo_EDITED.txt")
...
dat <- read.delim("Hanyuda et al 2018 Supp TableS9_Mutimo_EDITED.txt")[,c("Name1","Haplotype","Number")]
...
save(native,file="mcylforRangerCrossValidation.rda")

### ran on "XXXXCrossvalidation_forServer.R" separately 
load(file="mcyl_Ranger_out.rda")
...
mat <- as.matrix(tbl)
mat <- mat+0.01
cols.to.use <- c(blue2red(5),"black","darkgrey")
...
dev.off()
write.csv(as.matrix(tbl),"mcylByReg.csv")
...
write.csv(n,"mcyl_sampleSize.csv",quote=F)
