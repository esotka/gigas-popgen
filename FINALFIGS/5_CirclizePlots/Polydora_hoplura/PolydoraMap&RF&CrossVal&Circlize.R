...
meta <- read.csv("Polydora_meta.csv")
...
dat <- read.csv("./Polydora_inds_final.csv")
md2 = table(dat[,c("popID","AccessionNum")])
reg <- meta$source[match(rownames(md2),meta$pop)]

native_data = md2[!reg%in%c("EU","Cali"),]
...
intro_data =  md2[reg%in%c("EU","Cali"),]
...
save(native,file="PolydoraForRangerCrossValidation.rda")

### ran on "XXXXCrossvalidation_forServer.R" separately 
load(file="Polydora_Ranger_out.rda")
...
mat <- as.matrix(tbl)
mat <- mat+0.01
cols.to.use <- c(blue2red(5)[-1],"darkgrey","black")
#cols.to.use <- c(blue2red(5),"darkgrey","black")
#cols.to.use <- c(blue2red(5)[-1],rep("grey",ncol(mat))) # "black",
#rownames(mat) <- c("Miyagi","Tokyo","Seto Inland Sea","Kagoshima","Korea / western Japan")


pdf("circlize_noLabels.pdf",width=5,height=5)#,units="in")
circos.clear()
