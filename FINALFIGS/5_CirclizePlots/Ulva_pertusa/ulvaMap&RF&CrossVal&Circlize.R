...
meta <- read.delim("Hanyuda et al 2018 Supp TableS6 Ulva_edited.txt")
...
dat <- read.delim("Hanyuda et al 2018 Supp TableS6 Ulva_edited.txt")[,c("Name1","Haplotype","Number")]
...
save(native,file="ulvaforRangerCrossValidation.rda")

### ran on "XXXXCrossvalidation_forServer.R" separately 
load(file="ulva_Ranger_out.rda")
...
#mat <- as.matrix(tbl)
mat = as.matrix(data.frame(rowSums(tbl[,1:2]),tbl[,3])); colnames(mat) = c("Europe","Cali")
mat <- mat+0.01
...
write.csv(n,"ulva_sampleSize.csv",quote=F)
