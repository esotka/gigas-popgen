...
pdf("FINALFIGS/5_RandomForestCirclize/battr/map.pdf",height=6,width=5); map1(); dev.off()
png("FINALFIGS/5_RandomForestCirclize/battr/map.png",height=5,width=5,units="in",res=400); map1(); dev.off()
...
save(native,file="FINALFIGS/5_RandomForestCirclize/crossValidation/battrForRangerCrossValidation.rda")

### ran on "XXXXCrossvalidation_forServer.R" separately 
load(file="FINALFIGS/5_RandomForestCirclize/crossValidation/battr_Ranger_out.rda")
...
save(file="FINALFIGS/5_RandomForestCirclize/battr/region_assignment.rda",tbl)
pdf("FINALFIGS/5_RandomForestCirclize/battr/region_assignment.pdf")
par(mar=c(6,4,2,2))
heatmap(tbl)
dev.off()
