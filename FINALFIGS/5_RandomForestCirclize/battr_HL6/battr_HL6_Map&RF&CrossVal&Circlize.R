...
pdf("FINALFIGS/5_RandomForestCirclize/battr_HL6/map.pdf",height=6,width=5); map1(); dev.off()
png("FINALFIGS/5_RandomForestCirclize/battr_HL6/map.png",height=5,width=5,units="in",res=400); map1(); dev.off()

### 2) generate the random forest - native pops vs introduced pops
dat <- as.data.frame(read_xlsx('FINALFIGS/5_RandomForestCirclize/battr_HL1/Miura2006haplotypes_haps.xlsx',sheet=3))
...
save(native,file="FINALFIGS/5_RandomForestCirclize/crossValidation/battr_HL6ForRangerCrossValidation.rda")

### ran on "XXXXCrossvalidation_forServer.R" separately 
load(file="FINALFIGS/5_RandomForestCirclize/crossValidation/battr_HL6_Ranger_out.rda")
...
save(file="FINALFIGS/5_RandomForestCirclize/battr_HL6/region_assignment.rda",tbl)
pdf("FINALFIGS/5_RandomForestCirclize/battr_HL6/region_assignment.pdf")
par(mar=c(6,4,2,2))
heatmap(tbl)
dev.off()
...
pdf("FINALFIGS/5_RandomForestCirclize/battr_HL6/circlize.pdf",width=10,height=10)
circos.clear()
...
write.csv(mat,"FINALFIGS/5_RandomForestCirclize/ALLSPECIES/battr_HL6ByReg.csv")
