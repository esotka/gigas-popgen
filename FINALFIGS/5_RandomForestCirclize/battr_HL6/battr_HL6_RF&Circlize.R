...
dat <- as.data.frame(read_xlsx('FINALFIGS/5_RandomForestCirclize/battr/Miura2006haplotypes_haps.xlsx',sheet=3))
...
pdf("FINALFIGS/5_RandomForestCirclize/battr_HL6/random_forest.pdf")
#par(mar=c(7,4,4,4)+0.1)
heatmap(tbl)#,cexCol=1.7,cexRow=1.7)
heatmap(tbl[rowSums(tbl)>0,])#,cexCol=1.7,cexRow=1.7)
dev.off()

write.csv(tbl,"FINALFIGS/5_RandomForestCirclize/battr_HL6/RFprediction.csv",quote=F)
