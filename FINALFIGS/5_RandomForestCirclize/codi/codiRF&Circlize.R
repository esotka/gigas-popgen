...
meta$sourceID[meta$PopID%in%c("Hokkaido_Rishiri","Hokkaido_Wakkanai")] <- "hok"
meta$sourceID[meta$PopID%in%c("Washington_San_Juan_Island","Oregon_Coos_Bay")] <- "NAm_north"
...
dat <- t(tmp[,colnames(tmp)%in%LETTERS[1:9]]) # hap = row; pop=col
...
indID <- paste(popID,1:length(hapInd),sep=":")
#country <- ifelse(lonInd>0,"1_Asia","2_wNA")
metaInd <- data.frame(indID,hapInd)
md2 <- as.matrix(table(metaInd))
md2_pop <- unlist(lapply(strsplit(rownames(md2),":"),"[[",1))
md2_source <- meta$sourceID[match(md2_pop,meta$PopID)]

native_data = md2[!md2_source=="NAm_north",]
native_pops = as.factor(md2_pop[!md2_source=="NAm_north"])
intro_data =  md2[md2_source=="NAm_north",]
intro_pops =  as.factor(md2_pop[md2_source=="NAm_north"])
...
rowReg <- factor(rowReg); rowReg <- factor(rowReg,levels(rowReg)[c(1,4,2,3)])
...
datByReg2 <- as.matrix(data.frame(NAm_north=rowSums(datByReg)))
rownames(datByReg2) <- c("Hokkaido","Tokyo","Kagoshima","Korea / wJapan")
