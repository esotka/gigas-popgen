...
library(caret)
library(ggplot2)
load("GvermforRangerCrossValidation.rda")

fitControl=trainControl(method="repeatedcv", number=10,repeats=10) #10-fold cv repeated 10 times
rangerGrid = expand.grid(mtry = round(seq(2,0.99*ncol(native),length=5)),splitrule=c("gini","extratrees"),min.node.size=c(1,3,5,10) )
...
save(rfits,"gverm_Ranger_out.rda")
