...
meta <- as.data.frame(read_xlsx("FINALFIGS/5_RandomForestCirclize/sang/Blakeslee_Table1.xlsx",sheet = 1))
...
meta <- meta[!meta$Site.Abb=="TTT",]
meta$sourceID[meta$Site.Abb=="GER"] = "Europe"
...
md2_source <- meta$sourceID[match(md2_pop,meta$Site.Abb)]

native_data = md2[!md2_source=="Europe",]
native_pops = as.factor(md2_pop[!md2_source=="Europe"])
...
intro_data =  md2[md2_source=="Europe",]
intro_pops =  as.factor(md2_pop[md2_source=="Europe"])
intro_reg = as.factor(meta$sourceID[match(intro_pops,meta$Site.Abb)])
...
save(native,file="FINALFIGS/5_RandomForestCirclize/crossValidation/sangForRangerCrossValidation.rda")

### ran on "XXXXCrossvalidation_forServer.R" separately 
load(file="FINALFIGS/5_RandomForestCirclize/crossValidation/sang_Ranger_out.rda")
...
save(file="FINALFIGS/5_RandomForestCirclize/sang/region_assignment.rda",tbl)
#pdf("FINALFIGS/5_RandomForestCirclize/sang/region_assignment.pdf")
#par(mar=c(6,4,2,2))
#heatmap(tbl)
#dev.off()
...
pdf("FINALFIGS/5_RandomForestCirclize/sang/circlize.pdf",width=10,height=10)
circos.clear()
...
write.csv(mat,"FINALFIGS/5_RandomForestCirclize/ALLSPECIES/sangByReg.csv")
...
write.csv(n,"FINALFIGS/5_RandomForestCirclize/ALLSPECIES/sang_sampleSize.csv",quote=F)
