...
meta <- as.data.frame(read_xlsx("FINALFIGS/5_RandomForestCirclize/taka/Makino 2018 Table S1.xlsx",sheet = 1))[,1:6]
...
meta <- meta[!meta$Pop=="pop1",]
meta$sourceID[meta$Pop%in%c("pop53","pop54","pop55","pop56","pop57")] = "NorthEurope"
meta$sourceID[meta$Pop%in%c("pop58","pop59","pop60")] = "SouthEurope"
...
#md <- md[!md$variable=="TTT",]
popID <- rep(as.character(md$pop),md$n)
hapInd <- rep(md$haplotype,md$n)
...
native_data = md2[!md2_source%in%c("NorthEurope","SouthEurope"),]
native_pops = as.factor(md2_pop[!md2_source%in%c("NorthEurope","SouthEurope")])
...
intro_data =  md2[md2_source%in%c("NorthEurope","SouthEurope"),]
intro_pops =  as.factor(md2_pop[md2_source%in%c("NorthEurope","SouthEurope")])
...
save(native,file="FINALFIGS/5_RandomForestCirclize/crossValidation/takaForRangerCrossValidation.rda")

### ran on "XXXXCrossvalidation_forServer.R" separately 
load(file="FINALFIGS/5_RandomForestCirclize/crossValidation/taka_Ranger_out.rda")
...
save(file="FINALFIGS/5_RandomForestCirclize/taka/region_assignment.rda",tbl)
#pdf("FINALFIGS/5_RandomForestCirclize/taka/region_assignment.pdf")
#par(mar=c(6,4,2,2))
#heatmap(tbl)
#dev.off()
...
pdf("FINALFIGS/5_RandomForestCirclize/taka/circlize.pdf",width=10,height=10)
circos.clear()
...
write.csv(mat,"FINALFIGS/5_RandomForestCirclize/ALLSPECIES/takaByReg.csv")
...
write.csv(n,"FINALFIGS/5_RandomForestCirclize/ALLSPECIES/taka_sampleSize.csv",quote=F)
