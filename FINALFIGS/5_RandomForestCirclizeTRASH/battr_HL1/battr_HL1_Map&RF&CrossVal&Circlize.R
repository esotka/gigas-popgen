...
meta <- as.data.frame(read_xlsx("FINALFIGS/5_RandomForestCirclize/battr_HL1/Miura2006haplotypes_meta.xlsx",sheet = 1))
...
pdf("FINALFIGS/5_RandomForestCirclize/battr_HL1/map.pdf",height=6,width=5); map1(); dev.off()
png("FINALFIGS/5_RandomForestCirclize/battr_HL1/map.png",height=5,width=5,units="in",res=400); map1(); dev.off()

### 2) generate the random forest - native pops vs introduced pops
dat <- as.data.frame(read_xlsx('FINALFIGS/5_RandomForestCirclize/battr_HL1/Miura2006haplotypes_haps.xlsx',sheet=2))
...
native_reg = factor(native_reg,levels=levels(native_reg)[c(1,5,4,2,3)])
...
save(native,file="FINALFIGS/5_RandomForestCirclize/crossValidation/battr_HL1ForRangerCrossValidation.rda")

### ran on "XXXXCrossvalidation_forServer.R" separately 
load(file="FINALFIGS/5_RandomForestCirclize/crossValidation/battr_HL1_Ranger_out.rda")
...
save(file="FINALFIGS/5_RandomForestCirclize/battr_HL1/region_assignment.rda",tbl)
pdf("FINALFIGS/5_RandomForestCirclize/battr_HL1/region_assignment.pdf")
par(mar=c(6,4,2,2))
heatmap(tbl)
dev.off()
...
mat <- as.matrix(tbl)
mat <- mat+0.01
cols.to.use <- c(blue2red(5)[-1],"black",rep("grey",ncol(mat)))
#rownames(mat) <- c("Miyagi","Tokyo","Seto Inland Sea","Kagoshima","Korea / western Japan")


pdf("FINALFIGS/5_RandomForestCirclize/battr_HL1/circlize.pdf",width=10,height=10)
circos.clear()
...
write.csv(mat,"FINALFIGS/5_RandomForestCirclize/ALLSPECIES/battr_HL1ByReg.csv")
...
write.csv(n,"FINALFIGS/5_RandomForestCirclize/ALLSPECIES/battr_HL1_sampleSize.csv",quote=F)
