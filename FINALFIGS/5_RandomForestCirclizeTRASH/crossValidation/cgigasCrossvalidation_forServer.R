...
rangerGrid = expand.grid(mtry = round(seq(.1*ncol(native),.99*ncol(native),length=5)),splitrule=c("gini","extratrees"),min.node.size=c(1,3,5,10) )
...
save(rfits,file = "cgiga_Ranger_out.rda")
...
save(rf,file="cgiga_Ranger_out_rf.rda")
