...
meta <- as.data.frame(read_xlsx("FINALFIGS/5_RandomForestCirclize/dide/Stefaniak_AppTable1_summary.xlsx",sheet = 1))
meta <- meta[!meta$Pop=="Whiting Harbor Sitka AK",] # remove 
...
meta$sourceID[meta$Pop=="Asamushi Japan"] = "hon" # right on the border
...
tmpreg <- matrix(c(
  ...
  "UK","EuropeNorth",
  "NL","EuropeNorth",
  "NZ","NewZealand"),nrow=9,ncol=2,byrow = T)
tmpreg2 <-tmpreg[match(meta$Coastline,tmpreg[,1]),2]
...
pdf("FINALFIGS/5_RandomForestCirclize/dide/map.pdf",height=6,width=5); map1(); dev.off()
png("FINALFIGS/5_RandomForestCirclize/dide/map.png",height=5,width=5,units="in",res=400); map1(); dev.off()


### 2) generate the random forest - native pops vs introduced pops
#dat <- as.data.frame(read_xlsx('FINALFIGS/5_RandomForestCirclize/dide/Stefaniak_AppTable1_summary.xlsx',sheet=1))
dat <- meta[,!colnames(meta)%in%c("Coastline","Latitude","Longitude","gridIDs","sourceID")]
dat[is.na(dat)] = 0
pop <- dat$Pop
...
md2_source <- meta$sourceID[match(md2_pop,meta$Pop)]

native_data = md2[md2_source%in%c("hok","hon","tok"),]
native_pops = as.factor(md2_pop[md2_source%in%c("hok","hon","tok")])
native_reg = as.factor(meta$sourceID[match(native_pops,meta$Pop)])
native_reg = factor(native_reg,levels=levels(native_reg)[c(1:3)])
intro_data =  md2[!md2_source%in%c("hok","hon","tok"),]
intro_pops =  as.factor(md2_pop[!md2_source%in%c("hok","hon","tok")])
intro_reg = as.factor(meta$sourceID[match(intro_pops,meta$Pop)])
...
save(native,file="FINALFIGS/5_RandomForestCirclize/crossValidation/dideForRangerCrossValidation.rda")

### ran on "XXXXCrossvalidation_forServer.R" separately 
load(file="FINALFIGS/5_RandomForestCirclize/crossValidation/dide_Ranger_out.rda")
...
save(file="FINALFIGS/5_RandomForestCirclize/dide/region_assignment.rda",tbl)
pdf("FINALFIGS/5_RandomForestCirclize/dide/region_assignment.pdf")
par(mar=c(6,4,2,2))
heatmap(tbl)
dev.off()
...
mat <- as.matrix(tbl)
mat <- mat+0.01
cols.to.use <- c(blue2red(5)[-(4:5)],rep("grey",ncol(mat)))
#rownames(mat) <- c("Miyagi","Tokyo","Seto Inland Sea","Kagoshima","Korea / western Japan")


pdf("FINALFIGS/5_RandomForestCirclize/dide/circlize.pdf",width=10,height=10)
circos.clear()
...
write.csv(mat,"FINALFIGS/5_RandomForestCirclize/ALLSPECIES/dideByReg.csv")
...
write.csv(n,"FINALFIGS/5_RandomForestCirclize/ALLSPECIES/dide_sampleSize.csv",quote=F)
